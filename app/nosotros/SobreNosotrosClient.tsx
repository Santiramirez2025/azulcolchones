// app/sobre-nosotros/SobreNosotrosClient.tsx - CLIENT COMPONENT OPTIMIZED ⚡
'use client'

import { memo } from 'react'
import Link from 'next/link'
import { 
  Heart, Shield, Truck, Users, Award, Target, 
  CheckCircle, Star, MapPin, Calendar, Home 
} from 'lucide-react'

// ============================================================================
// MEMOIZED ICON WRAPPER - Prevents re-renders
// ============================================================================
...
const HeroSection = memo(() => (
  <header className="text-center mb-20">
    <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-violet-500/20 to-fuchsia-500/20 rounded-2xl mb-6 border border-violet-500/30">
      <Heart className="w-10 h-10 text-violet-400" aria-hidden="true" />
    </div>
...
// ============================================================================
// HISTORIA SECTION - Memoized
// ============================================================================
const HistoriaSection = memo(() => (
  <article className="bg-gradient-to-br from-zinc-900 to-zinc-950 border border-white/10 rounded-3xl p-8 md:p-12 mb-16">
    <div className="flex items-start gap-6">
      <div className="w-16 h-16 bg-gradient-to-br from-violet-500/20 to-fuchsia-500/20 rounded-2xl flex items-center justify-center flex-shrink-0 border border-violet-500/30">
        <Target className="w-8 h-8 text-violet-400" aria-hidden="true" />
      </div>
      <div>
...
const StatsSection = memo(() => {
  ...
  return (
    <section className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16" aria-label="Estadísticas">
      {stats.map((stat, index) => (
        <article 
          key={index}
          className="bg-gradient-to-br from-zinc-900 to-zinc-950 border border-white/10 rounded-2xl p-6 text-center hover:border-violet-500/30 transition-all hover:scale-105 will-change-transform"
        >
...
// ============================================================================
// SHOWROOM LOCATION - Memoized
// ============================================================================
const ShowroomSection = memo(() => (
  <section 
    className="bg-gradient-to-r from-violet-500/10 to-fuchsia-500/10 border border-violet-500/30 rounded-3xl p-8 md:p-12 mb-16"
  ...
  >
    <div className="flex flex-col md:flex-row items-center gap-8">
      <div className="w-20 h-20 bg-gradient-to-br from-violet-500/20 to-fuchsia-500/20 rounded-2xl flex items-center justify-center flex-shrink-0 border border-violet-500/30">
        <MapPin className="w-10 h-10 text-violet-400" aria-hidden="true" />
      </div>
      <div className="flex-1 text-center md:text-left">
        <h2 id="showroom-heading" className="text-3xl font-bold text-white mb-3">
          Visitá nuestro showroom
        </h2>
        <p className="text-zinc-300 text-lg mb-4">
          Te esperamos en nuestro local para que puedas probar y ver todos nuestros colchones
        </p>
        <address className="not-italic flex flex-col gap-2 text-zinc-300">
          ...
          <div className="flex items-center gap-2 justify-center md:justify-start">
            <Calendar className="w-5 h-5 text-violet-400" aria-hidden="true" />
      ...
      <a
        ...
        className="px-6 py-3 bg-white text-violet-600 rounded-xl font-bold hover:scale-105 transition-all shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-white will-change-transform"
...
// ============================================================================
// VALORES SECTION - Memoized
// ============================================================================
const ValoresSection = memo(() => {
  const valores = [
    {
      icon: Heart,
  ...
  return (
    <section className="mb-16" aria-labelledby="valores-heading">
      <h2 id="valores-heading" className="text-3xl font-bold text-white text-center mb-12">
        Nuestros valores
      </h2>
      <div className="grid md:grid-cols-2 gap-6">
        {valores.map((valor, index) => (
          <article 
            key={index}
            className={`bg-gradient-to-br ${valor.color} border ${valor.borderColor} rounded-2xl p-8 hover:scale-[1.02] transition-all will-change-transform`}
          >
            <div className={`w-14 h-14 bg-gradient-to-br ${valor.color} rounded-xl flex items-center justify-center mb-4 border ${valor.borderColor}`}>
              <valor.icon className={`w-7 h-7 ${valor.iconColor}`} aria-hidden="true" />
            </div>
...
const TimelineSection = memo(() => {
  ...
  return (
    <section 
      className="bg-gradient-to-br from-zinc-900 to-zinc-950 border border-white/10 rounded-3xl p-8 md:p-12 mb-16"
      aria-labelledby="timeline-heading"
    >
      <h2 id="timeline-heading" className="text-3xl font-bold text-white text-center mb-12">
        Nuestra trayectoria
      </h2>
      <div className="space-y-8" role="list">
        {hitos.map((hito, index) => (
          <article key={index} className="flex gap-6 items-start" role="listitem">
            <div className="flex-shrink-0">
              <div className="w-20 h-20 bg-gradient-to-br from-violet-500/20 to-fuchsia-500/20 border border-violet-500/30 rounded-xl flex items-center justify-center">
                <time className="text-violet-400 font-black text-lg" dateTime={hito.year}>
                  {hito.year}
                </time>
              </div>
            </div>
...
// ============================================================================
// WHY CHOOSE US SECTION - Memoized
// ============================================================================
const WhyChooseUsSection = memo(() => {
  ...
  return (
    <section 
      className="bg-gradient-to-br from-violet-500/10 to-fuchsia-500/10 border border-violet-500/30 rounded-3xl p-8 md:p-12"
      aria-labelledby="why-heading"
    >
      <div className="flex items-start gap-6 mb-8">
        <div className="w-16 h-16 bg-gradient-to-br from-violet-500/20 to-fuchsia-500/20 rounded-2xl flex items-center justify-center flex-shrink-0 border border-violet-500/30">
          <Award className="w-8 h-8 text-violet-400" aria-hidden="true" />
        </div>
        <div>
