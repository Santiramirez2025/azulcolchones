// app/terminos/TerminosClient.tsx - CLIENT OPTIMIZED ⚡
'use client'

import { memo, useMemo } from 'react'
import Link from 'next/link'
import { 
  FileText, ShoppingBag, Truck, RotateCcw, Shield, Copyright, 
  Scale, Mail, MessageCircle 
} from 'lucide-react'
...
const HeaderSection = memo(() => (
  <header className="text-center mb-16">
    ...
    <p className="text-zinc-500 text-sm">
      Última actualización: <time dateTime="2024-11-01">Noviembre 2024</time>
    </p>
  </header>
))
HeaderSection.displayName = 'HeaderSection'

const ImportantNotice = memo(() => (
...
const SectionCard = memo(({ section }: { section: Section }) => (
  <article className="bg-gradient-to-br from-zinc-900 to-zinc-950 border border-white/10 rounded-2xl p-6 hover:border-cyan-500/30 transition-all">
    <div className="flex items-center gap-3 mb-4">
      <div className="w-12 h-12 bg-cyan-500/10 rounded-xl flex items-center justify-center flex-shrink-0">
        <section.icon className="w-6 h-6 text-cyan-400" aria-hidden="true" />
      </div>
...
const ContactSection = memo(() => (
  <section 
    className="bg-gradient-to-r from-cyan-500/10 to-blue-500/10 border border-cyan-500/20 rounded-2xl p-8 mb-8"
  ...
  >
    ...
    <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
      ...
      <a 
        ...
        className="inline-flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-6 py-3 rounded-xl font-semibold transition-all border border-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400"
      >
        <Mail className="w-5 h-5" aria-hidden="true" />
        info@azulcolchones.com.ar
      </a>
    </div>
  </section>
))
...
const LegalLinks = memo(() => (
  <nav className="grid sm:grid-cols-3 gap-4 mb-8" aria-label="Enlaces legales">
    <a 
      href="https://www.argentina.gob.ar/defensadelconsumidor"
      target="_blank"
      rel="noopener noreferrer"
      className="bg-zinc-900 border border-white/10 rounded-xl p-4 hover:border-blue-500/30 transition-all text-center group focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
    >
      <Shield className="w-8 h-8 text-blue-400 mx-auto mb-2" aria-hidden="true" />
    ...
    <Link 
      href="/privacidad"
      className="bg-zinc-900 border border-white/10 rounded-xl p-4 hover:border-blue-500/30 transition-all text-center group focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500"
    >
      <Scale className="w-8 h-8 text-cyan-400 mx-auto mb-2" aria-hidden="true" />
    ...
    <Link 
      ...
      className="bg-zinc-900 border border-white/10 rounded-xl p-4 hover:border-blue-500/30 transition-all text-center group focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500"
    >
      <FileText className="w-8 h-8 text-violet-400 mx-auto mb-2" aria-hidden="true" />
...
const FooterNote = memo(() => (
  <footer className="bg-zinc-900/50 border border-white/10 rounded-xl p-6 text-center">
    ...
    <div className="mt-4 pt-4 border-t border-white/10">
      <address className="text-xs text-zinc-500 not-italic">
        Domicilio Legal: Balerdi 855, Villa María, Córdoba, Argentina
      </address>
    </div>
  </footer>
))
...
const MobileBottomBar = memo(() => (
  <aside className="fixed bottom-0 left-0 right-0 bg-gradient-to-r from-blue-600 to-cyan-500 p-4 lg:hidden z-50 shadow-2xl">
  ...
  const sections: Section[] = useMemo(() => [
    {
      ...
      content: 'Al acceder y utilizar este sitio web (azulcolchones.com.ar), aceptás estar sujeto a estos términos y condiciones de uso, nuestra política de privacidad y todas las leyes aplicables de la República Argentina. Si no estás de acuerdo con estos términos, por favor no utilices nuestro sitio.'
    },
    {
      icon: ShoppingBag,
    ...
    <main className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950 pt-32 pb-20">
      <div className="container mx-auto px-4 max-w-5xl">
        ...
        <section 
          ...
          aria-label="Términos y condiciones detallados"
        >
